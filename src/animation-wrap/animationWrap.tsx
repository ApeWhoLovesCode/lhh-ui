...
import { getTransformCurtain, handleLinearGradient } from "./utils";
...
const AnimationWrap = forwardRef<AnimationWrapInstance, AnimationWrapProps>(
  (comProps, ref) => {
    const props = useMergeProps<AnimationWrapProps, RequireType>(
      comProps,
      defaultProps
    );
    const {
    ...
    const renderAnimation = () => {
      if (name === "curtain") {
        return (
          <div
            className={classBem(`${classPrefix}-curtain`, {
              [position]: true,
            })}
            style={{
              transitionDuration: `${duration}ms`,
              transform: getTransformCurtain({ distance, position }),
              background: `linear-gradient(to ${replaceLineToSpace(
                position
              )}, ${`${handleLinearGradient(
                background
              )} 10%, ${background} 50%, ${background})`}`,
            }}
          ></div>
        );
      }
